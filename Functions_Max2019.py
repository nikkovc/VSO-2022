...
#Max found a bug in the stiffness to slider conversion, May 2019. The VSPA study at UM should continue to use the old conversion, which will be corrected post-hoc
#Future studies should use the file 'slider2stiffness_Max2019'
slider2stiffness = numpy.genfromtxt('slider2stiffness_Max2019.csv', delimiter=',') #This file is for a cam that max made.  
...
def InSwingDetection(last_angles, cur_angle, CDflag, cur_stiff):
	...
	if all(i < angle_lim and i > -angle_lim for i in last_angles) and CDflag == 0:
		return True, last_angles, CDflag
	else:
		return False, last_angles, CDflag
...
def sliderPosition(x_des, gaintype = 'high'):
	...
	if gaintype == 'high':
		...
		K_i = 0.8
		K_d = 0.0001
	elif gaintype == 'low': #USE THIS FOR HOMING
	...
	if gaintype == 'low':
		...
		time_limit = 2 # Max time for position control loop to execute (safety)
